...
-- TODO: propagation of labels to the worker throws an error.
-- Need to investigate why this is happening; log_remote_commands
-- shows that the CREATE TABLE and ALTER TABLE commands use the
-- same name, but the SECURITY LABEL command errors out.
SET citus.log_remote_commands TO on;
...
-- Similar error happens with reference tables, so issue is unrelated
-- to shard names
...
SELECT node_type, result FROM get_citus_tests_label_provider_labels('tref.a1') ORDER BY node_type;
